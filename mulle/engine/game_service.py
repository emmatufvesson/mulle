...
class GameEngine:
    ...
    def __init__(self, seed: int = 42, ai_enabled: bool = True):
        ...
        self.ai = self.players[1] if ai_enabled else None
    ...
    def play_session(
        self,
        rounds: int,
        interactive_selector: ActionSelector | None = None,
        starter_idx: int = 0,
    ) -> SessionResult:
        ...
        for omg in range(rounds):
            ...
            for r in range(6):
                self.deal_hands()
                selector = interactive_selector if (interactive_selector and omg == 0 and r == 0) else None
                round_result = self.play_round(r, starter_idx=starting_player_idx, action_selector=selector)
